...
class mymodel(BaseModel):
    def __init__(self, opt):
        ...
        if self.is_train:
            ...
            # self.optimizer_G_SFT = torch.optim.SGD(optim_params_SFT, lr=train_opt['lr_G']*5, \
            #     weight_decay=wd_G)
            # self.optimizer_G_other = torch.optim.SGD(optim_params_other, lr=train_opt['lr_G'], \
            #     weight_decay=wd_G)
    ...
    def feed_data(self, data, need_HR=True):
        ...
        # self.fea = data['seg_fea'].to(self.device)    # 认为有语义先验时添加，问题在于后面有没有用到self.fea的地方？

        for fea in data['multiple_fea']:
            fea = fea.to(self.device)
        self.mfea = data['multiple_fea']
        # category
    ...
    def optimize_parameters(self, step):
        # l_zl = model_T and model
        # G
        self.optimizer_G_SFT.zero_grad()
        self.optimizer_G_other.zero_grad()

        self.fake_H = self.netG((self.var_L, self.mfea))
